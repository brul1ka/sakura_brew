...
from brul1ka import clear_screen
...
while True:
    ...
    event = randint(1, 5)
    if event == 1:
        print(f"{Fore.LIGHTBLUE_EX}В городе кофейный фестиваль! Вдвое больше клиентов сегодня!")
        VISITORS_AMOUNT *= 2
    elif event == 2:
        print(f"{Fore.LIGHTBLUE_EX}Плохая погода. Людей сегодня будет в два раза меньше.")
        VISITORS_AMOUNT //= 2
    elif event == 3:
        print(f"{Fore.LIGHTBLUE_EX}Повышение налогов! Сегодня налог увеличен в 1.5 раза.")
    ...
    if day1.lower() == 'y':
        while True:
            ...
            beans_buy_amount = input('Сколько грамм хотите купить? (Для отмены напишите "c"): ')
    ...
    while True:
        try:
            coffee_price_input = input('Цена чашки кофе? (в целых долларах и не больше пяти): ')

            coffee_price = int(coffee_price_input)

            if coffee_price > 5:
                print('Бро, сказали же: "до ПЯТИ". Не будь слишком наглым, пожалуйста.\n')
            elif coffee_price <= 0:
                print(f'Бесплатно? Нет. Ты, конечно, прости, но за такую щедрость тебя {Fore.RED}уволят.\n')
            else:
    ...
    coffee_sold = min(VISITORS_AMOUNT, COFFEE_BEANS // CONSUMPTION_OF_BEANS_PER_ONE_GLASS_OF_COFFEE)
    ...
    print(f'Ну что же, день подошёл к концу. Сегодня пришло {VISITORS_AMOUNT} посетителей.')
    print(f'Вы продали {coffee_sold} чашек кофе по ${COFFEE_PRICE} и заработали ${revenue}.')
